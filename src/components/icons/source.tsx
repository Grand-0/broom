import ArrowIcon from "./arrow.svg?react";
import BrandbookIcon from "./brandbook.svg?react";
import BroomIcon from "./broom.svg?react";
import CloseIcon from "./close.svg?react";
import DevelopingIcon from "./developing.svg?react";
import DocumentIcon from "./document.svg?react";
import InfoIcon from "./info.svg?react";
import SettingsIcon from "./settings.svg?react";
import HamburgerIcon from "./hamburger.svg?react";

export { ArrowIcon, BrandbookIcon, BroomIcon, CloseIcon, DevelopingIcon, DocumentIcon, HamburgerIcon, InfoIcon, SettingsIcon };
